...
if(notifBtn) {
  ...
  if(closeNotifModalBtn) closeNotifModalBtn.addEventListener('click', () => notifModal.classList.add('hidden'));
}
...
async function loadNotifications() {
  try {
    ...
    if (entries.length > 0) {
      ...
      if (newestId > lastReadId) notifBadge.classList.remove('hidden');
    }
  } catch (err) { console.warn("Notifications disabled (file not found)."); }
}
